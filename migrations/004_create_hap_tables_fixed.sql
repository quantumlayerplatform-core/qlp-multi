...
-- Create index for hap_custom_rules
CREATE INDEX IF NOT EXISTS idx_hap_rules_tenant ON hap_custom_rules (tenant_id, enabled);
...
-- Create index for hap_whitelist
CREATE INDEX IF NOT EXISTS idx_hap_whitelist_tenant ON hap_whitelist (tenant_id);
...
-- Create indexes for hap_user_risk_scores
CREATE INDEX IF NOT EXISTS idx_hap_risk_score ON hap_user_risk_scores (risk_score DESC);
CREATE INDEX IF NOT EXISTS idx_hap_flagged ON hap_user_risk_scores (flagged_for_review, updated_at DESC);
...
-- Create index for hap_statistics
CREATE INDEX IF NOT EXISTS idx_hap_stats_period ON hap_statistics (tenant_id, period_start DESC);
