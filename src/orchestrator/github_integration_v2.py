        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 201:
                    ...
                    raise ValueError(f"GitHub authentication failed: {error_data.get('message', 'Invalid token')}")
                elif response.status == 403:
                    error_data = await response.json()
                    raise ValueError(f"GitHub API forbidden: {error_data.get('message', 'Check permissions or rate limit')}")
                    ...
                    raise ValueError(f"Invalid repository data: {error_data}")
                else:
                    error = await response.text()
                    raise Exception(f"Failed to create repository: HTTP {response.status} - {error}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    error_data = await response.json()
                    raise ValueError(f"Invalid GitHub token: {error_data.get('message', 'Bad credentials')}")
                elif response.status == 403:
                    error_data = await response.json()
                    raise ValueError(f"GitHub API rate limit exceeded or insufficient permissions: {error_data.get('message', 'Forbidden')}")
                else:
                    error = await response.text()
                    raise Exception(f"Failed to get user information: HTTP {response.status} - {error}")
    ...
    async def push_capsule_atomic(
        ...
        try:
            ...
            if not self._token_validated:
                ...
                if not is_valid:
                    raise ValueError("GitHub token validation failed. Please check your token.")
